...
def enqueue_task(task: ResearchItem):
    ...
    try:
        ...
        logger.info(f"Enqueued task {task.id} for {task.github_repo}: {task.research_direction}")
        return {"status": "success", "message": "Task enqueued successfully", "task": task_data}
...
def dequeue_task():
    ...
    try:
        ...
        if task_str:
            ...
            logger.info(f"Dequeued task {task_id} for {task.get('github_repo')}: {task.get('research_direction')}")
...
def execute_task(task: ResearchItem):
    ...
    try:
        logger.info(f"Directly executing task for {task.github_repo}: {task.research_direction}")
        ...
        return {"status": "success", "message": "Task executed directly", "result": result}
    except Exception as e:
